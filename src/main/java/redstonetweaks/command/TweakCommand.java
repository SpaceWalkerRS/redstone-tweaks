...
public class TweakCommand {
	public static void registerCommand(CommandDispatcher<ServerCommandSource> dispatcher) {
		...
		ArrayList<Setting<?>> settings = Settings.getSettings("tweak");
	...
	private static int queryValue(ServerCommandSource source, Setting<?> setting) {
		source.sendFeedback(new TranslatableText("The %s setting is currently set to %s", setting.getCommandArgumentIdentifier(), setting.get()), false);
		return 1;
	}
	...
	private static int setValue(CommandContext<ServerCommandSource> context, Setting<?> setting) throws CommandSyntaxException {
		...
		context.getSource().sendFeedback(new TranslatableText("The %s setting has been set to %s", setting.getCommandArgumentIdentifier(), setting.getFromArgument(context, "value")), false);
		return 1;
	}
}
